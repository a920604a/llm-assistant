...
這就是 Observability / Monitoring 的價值所在。
...
| 類別              | 說明                | 觀察對象                                                                                                     |
| --------------- | ----------------- | -------------------------------------------------------------------------------------------------------- |
| **Metrics**     | 可量化指標，用來追蹤系統健康和性能 | Celery 任務成功率、API 請求次數/延遲、Redis/DB 讀寫次數/延遲、LLM 調用延遲與錯誤                                                    |
| **Tracing**     | 流程追蹤，用於定位瓶頸或異常    | RAG 問答流程（Query Rewrite → Hybrid Search → Rerank → LLM）與 Background Tasks（Ingest / Email Service） |
| **Alert / Log** | 異常檢測與通知           | 服務可用性（down）、API 過頻、資源異常（CPU/Memory/Disk）、Redis/DB 錯誤、異常日誌                                                |
...
* **Instrumentator / decorator**：程式內自動收集 Metrics，覆蓋 FastAPI 路由、Redis/DB、Celery 任務
* **Langfuse**
...
1. **程式內 Metrics 裝飾器很重要**：Instrumentator / decorator 可捕捉每個 FastAPI 路由、Redis、DB 調用，補足外部 exporter 無法覆蓋的細節。
...
這篇文章可以直接貼到知識平台，文字簡潔，結合專案實務經驗，附上圖表示意，讀者容易理解 Observability 在專案中的應用。
