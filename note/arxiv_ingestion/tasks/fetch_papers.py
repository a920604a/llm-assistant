...
async def fetch_papers_task(target_date: str, max_results: int = 5) -> Tuple[ArxivClient, List[ArxivPaper]]:
    settings = ArxivSettings()
    client = ArxivClient(settings)
    papers = await client.fetch_papers(from_date=target_date, to_date=target_date, max_results=max_results)
