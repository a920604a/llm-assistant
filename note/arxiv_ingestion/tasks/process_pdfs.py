...
@task(retries=2)
async def process_pdfs_task(client: ArxivClient, papers: PDFParserService, process_pdfs: bool = True):
    pdf_parser = PDFParserService()
    metadata_fetcher = MetadataFetcher(client, pdf_parser)  # client 已不需
    pdf_results = await metadata_fetcher.process_pdfs_batch(papers) if process_pdfs else {}
